		...
		if (fs->wflag) {	/* Write back dirty window if needed */
			...
			if (wsect < (fs->fatbase + fs->fsize)) {	/* In FAT area */
				...
				for (nf = fs->n_fats; nf > 1; nf--) {	/* Reflect the change to all FAT copies */
					wsect += fs->fsize;
					(void)disk_write(fs->drv, fs->win, wsect, 1);
				}
			}
		}
#endif
...
)
{
	...
	if (res == FR_OK) {
		...
		if (fs->fs_type == FS_FAT32 && fs->fsi_flag) {
			...
			(void)disk_write(fs->drv, fs->win, fs->fsi_sector, 1);
			fs->fsi_flag = 0;
		}
...
)
{
	...
	if (clst < 2 || clst >= fs->n_fatent) {	/* Check range */
		...
		case FS_FAT12 :
			...
			if (res != FR_OK) break;
			p = &fs->win[bc % SS(fs)];
			*p = (uint8_t)((clst & 1) ? ((*p & 0x0F) | ((uint8_t)val << 4)) : (uint8_t)val);
			bc++;
			fs->wflag = 1;
			res = move_window(fs, fs->fatbase + (bc / SS(fs)));
			if (res != FR_OK) break;
			p = &fs->win[bc % SS(fs)];
			*p = (uint8_t)((clst & 1) ? (uint8_t)(val >> 4) : ((*p & 0xF0) | ((uint8_t)(val >> 8) & 0x0F)));
			break;
...
)
{
	...
	if (clst == 0) {	/* Static table (root-dir in FAT12/16) */
		...
		ic = (uint16_t)(SS(dj->fs) / 32 * dj->fs->csize);	/* Entries per cluster */
...
)
{
	...
	i = (UINT)(((dir[LDIR_Ord] & 0xBF) - 1) * 13);	/* Get offset in the LFN buffer */
	s = 0; wc = 1;
	do {
...
)
{
	...
	i = (UINT)((ord - 1) * 13);				/* Get offset in the LFN buffer */
	s = wc = 0;
	do {
...
)
{
	...
	*path = &p[si];						/* Return pointer to the next segment */
	...
	if (p[si] == '.') { /* Is this a dot entry? */
		...
		*path = &p[si];									/* Return pointer to the next segment */
		sfn[NS] = (uint8_t)((c <= ' ') ? NS_LAST | NS_DOT : NS_DOT);	/* Set last segment flag if end of path */
		return FR_OK;
	}
#endif
	for (;;) {
	...
	*path = &p[si];						/* Return pointer to the next segment */
	c = (uint8_t)((c <= ' ') ? NS_LAST : 0);		/* Set last segment flag if end of path */
...
)
{
	...
	vol = (UINT)(p[0] - '0');					/* Is there a drive number? */
	...
	if (res == FR_OK) {
		...
		if (res == FR_OK) {
			...
			if (!dir) {						/* Is it a root directory? */
				...
				dir[DIR_Attr] = (uint8_t)((value & mask) | (dir[DIR_Attr] & (uint8_t)~mask));	/* Apply attribute change */
				dj.fs->wflag = 1;
				res = sync(dj.fs);
			}
		}
...
/* Start critical section that any interruption or error can cause cross-link */
					...
					if (res == FR_OK) {
						...
						dir[DIR_Attr] = (uint8_t)(buf[0] | AM_ARC);
						djo.fs->wflag = 1;
...
)
{
	...
	if (sfd) {
		...
		if (fmt != FS_FAT32)				/* System ID */
			tbl[4] = (uint8_t)((n_vol < 0x10000) ? 0x04 : 0x06);
		else
			tbl[4] = 0x0c;
	...
	if (fmt == FS_FAT32)					/* Write backup (VBR+6) */
		(void)disk_write(drv, tbl, b_vol + 6, 1);
	...
	if (fmt == FS_FAT32) {
		...
		(void)disk_write(drv, tbl, b_vol + 1, 1);	/* Write original (VBR+1) */
		(void)disk_write(drv, tbl, b_vol + 7, 1);	/* Write backup (VBR+7) */
	}
